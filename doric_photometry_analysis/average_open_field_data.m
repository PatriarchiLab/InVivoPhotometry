...
main_folder_with_data_subfolders = 'Y:\Chernysheva Maria\Fiber_Photometry_data\data for averaging\Open_Field\dLight13b';
...
%% 
n_polyfit = 3;
window_median = 1; %in the end the raw signal (window_median = 1) worked the best
dFF_normalized =[];
...
if group_number == 1 && mouse_number == number_of_mice{group_number}-1 %m021 had a huge artifact due to pc screen shot down
    drop_signal_index_delete = [];
    artifact_amplitude = 0.44;
    q=1;
    for ii=1:0.5*length(current_isosbestic_signal)
        if current_isosbestic_signal(ii)<artifact_amplitude||current_functional_signal(ii)<artifact_amplitude&&current_isosbestic_signal(ii+2)>artifact_amplitude
            drop_signal_index_delete(q)=ii;
            q=q+1;
        end
    end
    current_functional_signal(drop_signal_index_delete)=[];
    current_isosbestic_signal(drop_signal_index_delete)=[];
    current_time_vector(drop_signal_index_delete)=[];
end
...
sem_Mean_dFF_AverageInput = cell(1,number_of_different_groups);
...
for group_number=1:number_of_different_groups
    ...
    sem_Mean_dFF_AverageInput{group_number}=std(dFF_G_filtered_AllMice_Averages{group_number},1)/sqrt(size(dFF_G_filtered_AllMice_Averages{group_number},1));    
   
end
%% 
step_2plot = 400; %to reduce the output pdf file size let's plot each 400th point
window_smooth = step_2plot;
figure

for group_number = 1:number_of_different_groups
mean_input_2plot = smoothdata(Mean_dFF_AverageInput{group_number},MethodSmooth,window_smooth);
sem_input_2plot = smoothdata(sem_Mean_dFF_AverageInput{group_number},MethodSmooth,window_smooth);
H = shadedErrorBar(time_trial(1:step_2plot:end)/60,mean_input_2plot(1:step_2plot:end),sem_input_2plot(1:step_2plot:end),{'color', Color_Plot{group_number},'linewidth',line_width},transparent);
hold on
end
xline(0,':')
hold off
...
C = dFF_G_filtered_AllMice_Averages{group_number}(1:end,1:step_2plot:end);
x = time_trial(1:end,1:step_2plot:end)/60;
y = (1:size(C,1));
...
hold on
colorbar ('Ticks',[color_limits(1):2:color_limits(2)])
